...
class PL7:
    ...
    def run(self):
        ...
        model.setObjective(gp.quicksum(costs[i][j] * x[i, j] for i in range(self.num_companies) for j in range(self.num_projects) if not np.isnan(costs[i][j])), gp.GRB.MINIMIZE)

        # Add constraints
        # Une seule entreprise par projet
        for j in range(self.num_projects):
            model.addConstr(gp.quicksum(x[i, j] for i in range(self.num_companies) if not np.isnan(costs[i][j])) == 1)
        # Au max 2 projets par entreprise
        for i in range(self.num_companies):
            model.addConstr(gp.quicksum(x[i, j] for j in range(self.num_projects) if not np.isnan(costs[i][j])) <= 2)
            ...
            for i in range(self.num_companies):
                for j in range(self.num_projects):
                    if not np.isnan(costs[i][j]) and x[i, j].x > 0.5:
                        resultat.append((i+1, j+1))
            return resultat, model.objVal
...
if "__main__" == __name__:
    ...
    pl7 = PL7(costs)
    assignment, valeur_obj = pl7.run()
    if assignment:
        for i, j in assignment:
            print(f"Assigner le project {j} a la compagnie {i}")
        print("Valeur objective:", valeur_obj)
    else:
